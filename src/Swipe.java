...
public class Swipe {
    ...
    static final int stepTime = 1000;
    ...
    public void drawSwipe(Graphics g, int pers, BufferedImage image0, int preplace, int place, int dx) {
        ...
        if (isMoving[pers]) {
            ...
            double k = (curTime - startTime[pers]) * 1.0 / stepTime;//доля шага
            g.drawImage(image0, (int) ((polyax[p + 1] - polyax[p]) * k + polyax[p]) + dx, (int) ((polyay[p + 1] - polyay[p]) * k + polyay[p]), 40, 40, null);//уравнение прямой
            if (k >= 1) {
                last_places[pers]++;
